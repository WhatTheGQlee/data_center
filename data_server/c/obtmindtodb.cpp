...
int main(int argc, char* argv[]) {
  ...
  if (argc != 5) {
    ...
    printf(
        "Example:/home/gqlee/project/bin/procctl 10 "
        "/home/gqlee/project/bin/obtmindtodb /home/gqlee/project/tmp/surfdata "
        "\"127.0.0.1,root,123456,data_server,3306\" utf8 "
        "/home/gqlee/project/log/obtmindtodb.log\n\n");

    printf(
        "本程序用于把全国站点分钟观测数据保存到数据库的T_"
        "ZHOBTMIND表中，数据只插入，不更新。\n");
...
bool _obtmindtodb(char* pathname, char* connstr, char* charset) {
  ...
  while (true) {
    ...
    logfile.Write(
        "已处理文件%s（totalcount=%d,insertcount=%d），耗时%.2f秒。\n",
