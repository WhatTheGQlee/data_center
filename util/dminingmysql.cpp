...
struct st_arg {
  ...
  char fieldstr[501];  // 抽取数据的SQL语句输出结果集字段名，字段名用逗号分隔。
...
char strfieldname[MAXFIELDCOUNT][31];  // 结果集字段名数组 从starg.fieldstr解析
...
bool _dminingmysql() {
  ...
  char strfieldvalue[ifieldcount]
                    [MAXFIELDLEN +
                     1];  // 抽取数据的SQL执行后，存放结果集字段值的数组。
  ...
  while (true) {
    ...
    for (int ii = 1; ii <= ifieldcount; ii++)
      File.Fprintf("<%s>%s</%s>", strfieldname[ii - 1], strfieldvalue[ii - 1],
                   strfieldname[ii - 1]);
...
void _help() {
  ...
  printf(
      "Sample:/home/gqlee/project/bin/procctl 3600 "
      "/home/gqlee/project/bin/dminingmysql "
      "/home/gqlee/project/log/dminingmysql_ZHOBTCODE.log "
      "/home/gqlee/project/util/xmlbuffer/dminingmysql1.xml"
      "\"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>utf8</"
      "charset><selectsql>select obtid,cityname,provname,lat,lon,height from "
      "T_ZHOBTCODE</"
      "selectsql><fieldstr>obtid,cityname,provname,lat,lon,height</"
      "fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE</"
      "bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata</"
      "outpath><timeout>30</timeout><pname>dminingmysql_ZHOBTCODE</"
      "pname>\"\n\n");
  printf(
      "       /project/tools1/bin/procctl   30 "
      "/project/tools1/bin/dminingmysql /log/idc/dminingmysql_ZHOBTMIND.log "
      "\"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>utf8</"
      "charset><selectsql>select "
      "obtid,date_format(ddatetime,'%%%%Y-%%%%m-%%%%d "
      "%%%%H:%%%%i:%%%%s'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND where "
      "keyid>:1 and "
      "ddatetime>timestampadd(minute,-120,now())</"
      "selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</"
      "fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</"
      "fieldlen><bfilename>ZHOBTMIND</bfilename><efilename>HYCZ</"
      "efilename><outpath>/idcdata/dmindata</outpath><starttime></"
      "starttime><incfield>keyid</incfield><incfilename>/idcdata/dmining/"
      "dminingmysql_ZHOBTMIND_HYCZ.list</incfilename><timeout>30</"
      "timeout><pname>dminingmysql_ZHOBTMIND_HYCZ</pname><maxcount>1000</"
      "maxcount><connstr1>127.0.0.1,root,mysqlpwd,mysql,3306</connstr1>\"\n\n");

  printf(
      "本程序是数据中心的公共功能模块，用于从mysql数据库源表抽取数据，生成xml文"
      "件。\n");
  ...
  printf(
      "selectsql   "
      "从数据源数据库抽取数据的SQL语句，注意：时间函数的百分号%"
      "需要四个，显示出来才有两个，被prepare之后将剩一个。\n");
  printf(
      "fieldstr    "
      "抽取数据的SQL语句输出结果集字段名，中间用逗号分隔，将作为xml文件的字段名"
      "。\n");
  printf(
      "fieldlen    "
      "抽取数据的SQL语句输出结果集字段的长度，中间用逗号分隔。fieldstr与fieldle"
      "n的字段必须一一对应。\n");
  ...
  printf(
      "maxcount    "
      "输出xml文件的最大记录数，缺省是0，表示无限制，如果本参数取值为0，注意适"
      "当加大timeout的取值，防止程序超时。\n");
  printf(
      "starttime   "
      "程序运行的时间区间，例如02,"
      "13表示：如果程序启动时，踏中02时和13时则运行，其它时间不运行。"
      "如果starttime为空，那么starttime参数将失效，只要本程序启动就会执行数据抽"
      "取，为了减少数据源"
      "的压力，从数据库抽取数据的时候，一般在对方数据库最闲的时候时进行。\n");
  printf(
      "incfield    "
      "递增字段名，它必须是fieldstr中的字段名，并且只能是整型，一般为自增字段。"
      "如果incfield为空，表示不采用增量抽取方案。");
  printf(
      "incfilename "
      "已抽取数据的递增字段最大值存放的文件，如果该文件丢失，将重新抽取全部的数"
      "据。\n");
  printf(
      "connstr1    "
      "已抽取数据的递增字段最大值存放的数据库的连接参数。connstr1和incfilename"
      "二选一，connstr1优先。");
...
bool _xmltoarg(char* strxmlbuffer) {
  ...
  GetXMLBuffer(strxmlbuffer, "incfilename", starg.incfilename,
               300);  // 可选参数。
...
void crtxmlfilename()  // 生成xml文件名。
{
  ...
  static int iseq = 1;
  SNPRINTF(strxmlfilename, 300, sizeof(strxmlfilename), "%s/%s_%s_%s_%d.xml",
...
bool readincfield() {
  ...
  if (strlen(starg.connstr1) != 0) {
    // 从数据库表中加载自增字段的最大值。
    // create table T_MAXINCVALUE(pname varchar(50),maxincvalue
    // numeric(15),primary key(pname));
