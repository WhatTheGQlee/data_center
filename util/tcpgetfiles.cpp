...
struct st_arg {
  ...
  char srvpathbak[301];  // 文件下载后服务端文件备份的根目录，ptype==2时有效。
...
// 接收文件的内容。
bool RecvFile(const int sockfd,
              const char* filename,
              const char* mtime,
              int filesize);
...
void _help() {
  ...
  printf(
      "Sample:/project/tools1/bin/procctl 20 /project/tools1/bin/tcpgetfiles "
      "/log/idc/tcpgetfiles_surfdata.log "
      "\"<ip>192.168.174.132</ip><port>5005</port><ptype>1</ptype><srvpath>/"
      "tmp/tcp/surfdata2</srvpath><andchild>true</"
      "andchild><matchname>*.XML,*.CSV,*.JSON</matchname><clientpath>/tmp/tcp/"
      "surfdata3</clientpath><timetvl>10</timetvl><timeout>50</"
      "timeout><pname>tcpgetfiles_surfdata</pname>\"\n");
  printf(
      "       /project/tools1/bin/procctl 20 /project/tools1/bin/tcpgetfiles "
      "/log/idc/tcpgetfiles_surfdata.log "
      "\"<ip>192.168.174.132</ip><port>5005</port><ptype>2</ptype><srvpath>/"
      "tmp/tcp/surfdata2</srvpath><srvpathbak>/tmp/tcp/surfdata2bak</"
      "srvpathbak><andchild>true</andchild><matchname>*.XML,*.CSV,*.JSON</"
      "matchname><clientpath>/tmp/tcp/surfdata3</clientpath><timetvl>10</"
      "timetvl><timeout>50</timeout><pname>tcpgetfiles_surfdata</"
      "pname>\"\n\n\n");
  ...
  printf(
      "srvpathbak    "
      "文件成功下载后，服务端文件备份的根目录，当ptype==2时有效。\n");
  printf(
      "andchild      "
      "是否下载srvpath目录下各级子目录的文件，true-是；false-"
      "否，缺省为false。\n");
