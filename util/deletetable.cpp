...
void _help(char* argv[]) {
  ...
  printf(
      "Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/deletetable "
      "/log/idc/deletetable_ZHOBTMIND1.log "
      "\"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</"
      "connstr><tname>T_ZHOBTMIND1</tname><keycol>keyid</keycol><where>where "
      "ddatetime<timestampadd(minute,-120,now())</"
      "where><starttime>01,02,03,04,05,13</starttime><timeout>120</"
      "timeout><pname>deletetable_ZHOBTMIND1</pname>\"\n\n");
...
bool _deletetable() {
  ...
  stmtsel.prepare("select %s from %s %s", starg.keycol, starg.tname,
                  starg.where);
  ...
  stmtdel.prepare("delete from %s where %s in (%s)", starg.tname, starg.keycol,
                  bindstr);
  for (int ii = 0; ii < MAXPARAMS; ii++)
  ...
  if (stmtsel.m_cda.rpc > 0)
    logfile.Write("delete from %s %d rows in %.02fsec.\n", starg.tname,
                  stmtsel.m_cda.rpc, Timer.Elapsed());
