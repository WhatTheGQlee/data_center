...
void* pthmain(void* arg) {
  ...
  while (true) {
    ...
    if ((conn.m_cda.rc >= 2000) && (conn.m_cda.rc <= 2013))
      conn.disconnect();
...
When the cancelability type of a thread is set to PTHREAD_CAN_CEL_DEFERRED, a
...
cancellation  request  and instead  of  returning to the caller of
pthread_cond_timedwait() or pthread_cond_wait(), starts the thread cancellation
...
void _help(char* argv[]) {
  ...
  printf(
      "Sample:/project/tools/bin/procctl 10 /project/tools/bin/webserver1 "
      "/log/idc/webserver1.log "
      "\"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>utf8</"
      "charset><port>5008</port><timeout>5</timeout>\"\n\n");

  printf(
      "本程序是数据总线的服务端程序，为数据中心提供http协议的数据访问接口。\n");
...
// 从GET请求中获取参数。
bool getvalue(const char* buffer,
              const char* name,
              char* value,
              const int len) {
  value[0] = 0;
...
bool Login(connection* conn, const char* buffer, const int sockfd) {
  ...
  stmt.connect(conn);
  stmt.prepare(
      "select count(*) from T_USERINFO where username=:1 and passwd=:2 and "
      "rsts=1");
  ...
  if (icount != 1) {
    ...
    sprintf(strbuffer,
            ...
            "Server: webserver1\r\n"
            "Content-Type: text/html;charset=utf-8\r\n\r\n"
            "<retcode>-1</retcode><message>username or passwd is "
            "invailed</message>");
...
bool CheckPerm(connection* conn, const char* buffer, const int sockfd) {
  ...
  stmt.connect(conn);
  stmt.prepare(
      "select count(*) from T_USERANDINTER where username=:1 and intername=:2 "
...
bool ExecSQL(connection* conn, const char* buffer, const int sockfd) {
  ...
  stmt.connect(conn);
  stmt.prepare(
      "select selectsql,colstr,bindin from T_INTERCFG where intername=:1");
  ...
  while (true) {
    ...
    for (int ii = 0; ii < CmdStr.CmdCount(); ii++) {
      ...
      snprintf(strtemp, 2000, "<%s>%s</%s>", CmdStr.m_vCmdStr[ii].c_str(),
               colvalue[ii], CmdStr.m_vCmdStr[ii].c_str());
    ...
    for (int ii = 0; ii < vpthinfo.size(); ii++) {
      ...
      if ((now - vpthinfo[ii].atime) > 30) {
        logfile.Write("thread(%ld) timeout(%d).\n", vpthinfo[ii].pthid,
                      now - vpthinfo[ii].atime);
        // 杀掉线程。
        pthread_kill(vpthinfo[ii].pthid, SIGUSR1);

        // 重启线程。
        if (pthread_create(&vpthinfo[ii].pthid, NULL, pthmain,
                           (void*)(long)ii) != 0) {
          logfile.Write("pthread_create failed.\n");
          mainexit(-1);
        }
