...
void _help(char* argv[]) {
  ...
  printf(
      "Sample:/project/tools1/bin/procctl 3600 "
      "/project/tools1/bin/migratetable /log/idc/migratetable_ZHOBTMIND.log "
      "\"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</"
      "connstr><srctname>T_ZHOBTMIND</srctname><dsttname>T_ZHOBTMIND_HIS</"
      "dsttname><keycol>keyid</keycol><where>where "
      "ddatetime<timestampadd(minute,-120,now())</"
      "where><starttime>01,02,03,04,05,13</starttime><maxcount>300</"
      "maxcount><timeout>120</timeout><pname>migratetable_ZHOBTMIND</"
      "pname>\"\n\n");
  ...
  printf(
      "starttime   "
      "程序运行的时间区间，例如02,"
      "13表示：如果程序运行时，踏中02时和13时则运行，其它时间\n"
      "            "
      "不运行。如果starttime为空，本参数将失效，只要本程序启动就会执行数据迁移"
      "，为了减少\n"
...
bool _migratetable() {
  ...
  stmtsel.prepare("select %s from %s %s", starg.keycol, starg.srctname,
                  starg.where);
  ...
  stmtins.prepare("insert into %s(%s) select %s from %s where %s in (%s)",
                  starg.dsttname, TABCOLS.m_allcols, TABCOLS.m_allcols,
                  starg.srctname, starg.keycol, bindstr);

  sqlstatement stmtdel(&conn2);
  stmtdel.prepare("delete from %s where %s in (%s)", starg.dsttname,
                  starg.keycol, bindstr);
  ...
  if (stmtsel.m_cda.rpc > 0)
    logfile.Write("migrate %s to %s %d rows in %.02fsec.\n", starg.srctname,
                  starg.dsttname, stmtsel.m_cda.rpc, Timer.Elapsed());
