...
void _help(char* argv[]) {
  ...
  printf(
      "Sample:/project/tools1/bin/procctl 10 "
      "/project/tools1/bin/syncincrementex "
      "/log/idc/syncincrementex_ZHOBTMIND2.log "
      "\"<localconnstr>192.168.174.129,root,mysqlpwd,mysql,3306</"
      "localconnstr><remoteconnstr>192.168.174.132,root,mysqlpwd,mysql,3306</"
      "remoteconnstr><charset>utf8</charset><remotetname>T_ZHOBTMIND1</"
      "remotetname><localtname>T_ZHOBTMIND2</"
      ...
      "localcols><remotekeycol>keyid</remotekeycol><localkeycol>recid</"
      "localkeycol><timetvl>2</timetvl><timeout>50</"
      "timeout><pname>syncincrementex1_ZHOBTMIND2</pname>\"\n\n");

  printf(
      "       /project/tools1/bin/procctl 10 "
      "/project/tools1/bin/syncincrementex "
      "/log/idc/syncincrementex_ZHOBTMIND3.log "
      "\"<localconnstr>192.168.174.129,root,mysqlpwd,mysql,3306</"
      "localconnstr><remoteconnstr>192.168.174.132,root,mysqlpwd,mysql,3306</"
      "remoteconnstr><charset>utf8</charset><remotetname>T_ZHOBTMIND1</"
      "remotetname><localtname>T_ZHOBTMIND3</"
      "localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</"
      "remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</"
      "localcols><where>and obtid like "
      "'54%%%%'</where><remotekeycol>keyid</remotekeycol><localkeycol>recid</"
      "localkeycol><timetvl>2</timetvl><timeout>50</"
      "timeout><pname>syncincrementex1_ZHOBTMIND3</pname>\"\n\n");
  ...
  printf(
      "where         同步数据的条件，填充在select remotekeycol from "
      "remotetname where remotekeycol>:1之后，\n"
...
bool _syncincrementex(bool& bcontinue) {
  ...
  stmtsel.prepare("select %s from %s where %s>:1 %s order by %s",
                  starg.remotecols, starg.remotetname, starg.remotekeycol,
                  starg.where, starg.remotekeycol);
  stmtsel.bindin(1, &maxkeyvalue);
  for (int ii = 0; ii < colcount; ii++)
    stmtsel.bindout(ii + 1, colvalues[ii], TABCOLS.m_maxcollen);

  // 拼接插入SQL语句绑定参数的字符串 insert ... into starg.localtname
  // values(:1,:2,...:colcount)
  ...
  stmtins.prepare("insert into %s(%s) values(%s)", starg.localtname,
                  starg.localcols, bindstr);
  for (int ii = 0; ii < colcount; ii++) {
  ...
  if (stmtsel.m_cda.rpc > 0) {
    logfile.Write("sync %s to %s(%d rows) in %.2fsec.\n", starg.remotetname,
                  starg.localtname, stmtsel.m_cda.rpc, Timer.Elapsed());
