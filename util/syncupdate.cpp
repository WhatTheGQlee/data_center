...
void _help(char* argv[]) {
  ...
  printf(
      "Sample:/project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate "
      "/log/idc/syncupdate_ZHOBTCODE2.log "
      "\"<localconnstr>192.168.174.129,root,mysqlpwd,mysql,3306</"
      "localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTCODE1</"
      "fedtname><localtname>T_ZHOBTCODE2</"
      "localtname><remotecols>obtid,cityname,provname,lat,lon,height/"
      "10,upttime,keyid</"
      "remotecols><localcols>stid,cityname,provname,lat,lon,altitude,upttime,"
      "keyid</localcols><synctype>1</synctype><timeout>50</"
      "timeout><pname>syncupdate_ZHOBTCODE2</pname>\"\n\n");

  // 因为测试的需要，xmltodb程序每次会删除LK_ZHOBTCODE1中的数据，全部的记录重新入库，keyid会变。
  // 所以以下脚本不能用keyid，要用obtid，用keyid会出问题，可以试试。
  printf(
      "       /project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate "
      "/log/idc/syncupdate_ZHOBTCODE3.log "
      "\"<localconnstr>192.168.174.129,root,mysqlpwd,mysql,3306</"
      "localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTCODE1</"
      "fedtname><localtname>T_ZHOBTCODE3</"
      "localtname><remotecols>obtid,cityname,provname,lat,lon,height/"
      "10,upttime,keyid</"
      "remotecols><localcols>stid,cityname,provname,lat,lon,altitude,upttime,"
      "keyid</localcols><where>where obtid like "
      "'54%%%%'</where><synctype>2</"
      "synctype><remoteconnstr>192.168.174.132,root,mysqlpwd,mysql,3306</"
      "remoteconnstr><remotetname>T_ZHOBTCODE1</"
      "remotetname><remotekeycol>obtid</remotekeycol><localkeycol>stid</"
      "localkeycol><maxcount>10</maxcount><timeout>50</"
      "timeout><pname>syncupdate_ZHOBTCODE3</pname>\"\n\n");

  printf(
      "       /project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate "
      "/log/idc/syncupdate_ZHOBTMIND2.log "
      "\"<localconnstr>192.168.174.129,root,mysqlpwd,mysql,3306</"
      "localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTMIND1</"
      "fedtname><localtname>T_ZHOBTMIND2</"
      "localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</"
      "remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</"
      "localcols><where>where "
      "ddatetime>timestampadd(minute,-120,now())</where><synctype>2</"
      "synctype><remoteconnstr>192.168.174.132,root,mysqlpwd,mysql,3306</"
      "remoteconnstr><remotetname>T_ZHOBTMIND1</"
      "remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</"
      "localkeycol><maxcount>300</maxcount><timeout>50</"
      "timeout><pname>syncupdate_ZHOBTMIND2</pname>\"\n\n");
  ...
  printf(
      "remotecols    "
      "远程表的字段列表，用于填充在select和from之间，所以，remotecols可以是真实"
      "的字段，\n"
      "              "
      "也可以是函数的返回值或者运算结果。如果本参数为空，就用localtname表的字段"
      "列表填充。\n");
  printf(
      "localcols     "
      "本地表的字段列表，与remotecols不同，它必须是真实存在的字段。如果本参数为"
      "空，\n"
      "              就用localtname表的字段列表填充。\n");

  printf(
      "where         "
      "同步数据的条件，为空则表示同步全部的记录，填充在delete本地表和select "
      "Federated表\n"
      "              "
      "之后，注意：1）where中的字段必须同时在本地表和Federated表中；2）不要用系"
      "统时间作\n"
  ...
  printf(
      "maxcount      "
      "每批执行一次同步操作的记录数，不能超过MAXPARAMS宏（在_mysql."
      "h中定义），当synctype==2时有效。\n");
  ...
  printf(
      "pname         "
      "本程序运行时的进程名，尽可能采用易懂的、与其它进程不同的名称，方便故障排"
      "查。\n\n");
  printf(
      "注意：\n1）remotekeycol和localkeycol字段的选取很重要，如果用了MySQL的自"
      "增字段，那么在远程表中数据生成后自增字段的值不可改变，否则同步会失败；\n"
      "2）当远程表中存在delete操作时，无法分批同步，因为远程表的记录被delete后"
      "就找不到了，无法从本地表中执行delete操作。\n\n\n");
}
...
(
   ...
   keyid                int not null auto_increment comment
'记录编号，自动增长列。', primary key (obtid), unique key ZHOBTCODE1_KEYID
(keyid) )ENGINE=FEDERATED
CONNECTION='mysql://root:mysqlpwd@192.168.174.132:3306/mysql/T_ZHOBTCODE1';
...
(
   ...
   upttime              timestamp not null comment '更新时间。',
   keyid                bigint not null auto_increment comment
'记录编号，自动增长列。', primary key (obtid, ddatetime), unique key
ZHOBTMIND1_KEYID (keyid) )ENGINE=FEDERATED
CONNECTION='mysql://root:mysqlpwd@192.168.174.132:3306/mysql/T_ZHOBTMIND1';
*/
...
bool _syncupdate() {
  ...
  if (starg.synctype == 1) {
    ...
    if (stmtdel.execute() != 0) {
      logfile.Write("stmtdel.execute() failed.\n%s\n%s\n", stmtdel.m_sql,
                    stmtdel.m_cda.message);
      return false;
    }

    // 再把starg.fedtname表中满足where条件的记录插入到starg.localtname表中。
    stmtins.prepare("insert into %s(%s) select %s from %s %s", starg.localtname,
                    starg.localcols, starg.remotecols, starg.fedtname,
                    starg.where);
    if (stmtins.execute() != 0) {
      logfile.Write("stmtins.execute() failed.\n%s\n%s\n", stmtins.m_sql,
                    stmtins.m_cda.message);
      connloc
          .rollback();  // 如果这里失败了，可以不用回滚事务，connection类的析构函数会回滚。
      return false;
    }
  ...
  stmtsel.prepare("select %s from %s %s", starg.remotekeycol, starg.remotetname,
                  starg.where);
  ...
  stmtdel.prepare("delete from %s where %s in (%s)", starg.localtname,
                  starg.localkeycol, bindstr);
  for (int ii = 0; ii < starg.maxcount; ii++) {
    stmtdel.bindin(ii + 1, keyvalues[ii], 50);
  }

  // 准备插入本地表数据的SQL语句，一次插入starg.maxcount条记录。
  // insert into T_ZHOBTCODE3(stid
  // ,cityname,provname,lat,lon,altitude,upttime,keyid)
  //                   select
  ...
  stmtins.prepare("insert into %s(%s) select %s from %s where %s in (%s)",
                  starg.localtname, starg.localcols, starg.remotecols,
                  starg.fedtname, starg.remotekeycol, bindstr);
  for (int ii = 0; ii < starg.maxcount; ii++) {
  ...
  while (true) {
    ...
    if (ccount == starg.maxcount) {
      ...
      logfile.Write("sync %s to %s(%d rows) in %.2fsec.\n", starg.fedtname,
                    starg.localtname, ccount, Timer.Elapsed());
