...
struct st_columns
{
  ...
  int   collen;       //
列的长度，number固定20，date固定19，char的长度由表结构决定。 int   pkseq; //
...
public:
  ...
  char m_allcols[3001];  //
全部的字段名列表，以字符串存放，中间用半角的逗号分隔。 char m_pkcols[301];    //
主键字段名列表，以字符串存放，中间用半角的逗号分隔。
...
int totalcount, inscount,
    uptcount;  // xml文件的总记录数、插入记录数和更新记录数。
...
#define MAXCOLCOUNT \
  500  // 每个表字段的最大数，也可以用MAXPARAMS宏（在_mysql.h中定义）。
...
void _help(char* argv[]) {
  ...
  printf(
      "Sample:/home/gqlee/project/bin/procctl 10 "
      "/home/gqlee/project/bin/xmltodb "
      "/home/gqlee/project/log/xmltodb.log "
      "/home/gqlee/project/util/xmlbuffer/xmltodb.xml"
      "\"<connstr>127.0.0.1,root,mysqlpwd,mysql,3306</connstr><charset>utf8</"
      "charset><inifilename>/project/idc1/ini/xmltodb.xml</"
      "inifilename><xmlpath>/idcdata/xmltodb/vip1</xmlpath><xmlpathbak>/"
      "idcdata/xmltodb/vip1bak</xmlpathbak><xmlpatherr>/idcdata/xmltodb/"
      "vip1err</xmlpatherr><timetvl>5</timetvl><timeout>50</"
      "timeout><pname>xmltodb_vip1</pname>\"\n\n");
  ...
  printf(
      "charset     "
      "数据库的字符集，这个参数要与数据源数据库保持一致，否则会出现中文乱码的情"
      "况。\n");
  ...
  printf(
      "timetvl     本程序的时间间隔，单位：秒，视业务需求而定，2-30之间。\n");
  printf(
      "timeout     "
...
bool _xmltoarg(char* strxmlbuffer) {
  ...
  char tmp[1024];
  GetXMLBuffer(strxmlbuffer, "root", tmp);
  strcpy(strxmlbuffer, tmp);
...
bool _xmltodb() {
  int counter =
      50;  // 加载入库参数的计数器，初始化为50是为了在第一次进入循环的时候就加载参数。
...
bool loadxmltotable() {
  ...
  while (true) {
    ...
    GetXMLBuffer(strBuffer, "filename", stxmltotable.filename,
                 100);  // xml文件的匹配规则，用逗号分隔。
    GetXMLBuffer(strBuffer, "tname", stxmltotable.tname, 30);  // 待入库的表名。
    GetXMLBuffer(strBuffer, "uptbz",
                 &stxmltotable.uptbz);  // 更新标志：1-更新；2-不更新。
    GetXMLBuffer(strBuffer, "execsql", stxmltotable.execsql,
                 300);  // 处理xml文件之前，执行的SQL语句。
...
bool xmltobakerr(char* fullfilename, char* srcpath, char* dstpath) {
  ...
  UpdateStr(dstfilename, srcpath, dstpath,
            false);  // 小心第四个参数，一定要填false。
...
bool CTABCOLS::allcols(connection *conn,char *tablename)
{
  ...
  stmt.connect(conn);
  stmt.prepare("select
lower(column_name),lower(data_type),character_maximum_length from
information_schema.COLUMNS where table_name=:1"); stmt.bindin(1,tablename,30);
...
strcpy(stcolumns.datatype,"date"); if
(strcmp(stcolumns.datatype,"timestamp")==0) strcpy(stcolumns.datatype,"date");

    if (strcmp(stcolumns.datatype,"tinyint")==0)
strcpy(stcolumns.datatype,"number"); if
(strcmp(stcolumns.datatype,"smallint")==0)  strcpy(stcolumns.datatype,"number");
    if (strcmp(stcolumns.datatype,"mediumint")==0)
strcpy(stcolumns.datatype,"number"); if (strcmp(stcolumns.datatype,"int")==0)
strcpy(stcolumns.datatype,"number"); if
(strcmp(stcolumns.datatype,"integer")==0)   strcpy(stcolumns.datatype,"number");
    if (strcmp(stcolumns.datatype,"bigint")==0)
strcpy(stcolumns.datatype,"number"); if
(strcmp(stcolumns.datatype,"numeric")==0)   strcpy(stcolumns.datatype,"number");
    if (strcmp(stcolumns.datatype,"decimal")==0)
strcpy(stcolumns.datatype,"number"); if (strcmp(stcolumns.datatype,"float")==0)
strcpy(stcolumns.datatype,"number"); if (strcmp(stcolumns.datatype,"double")==0)
...
bool CTABCOLS::pkcols(connection *conn,char *tablename)
{
  ...
  sqlstatement stmt;
  stmt.connect(conn);
  stmt.prepare("select lower(column_name),seq_in_index from
information_schema.STATISTICS where table_name=:1 and index_name='primary' order
by seq_in_index"); stmt.bindin(1,tablename,30); stmt.bindout(1,
stcolumns.colname,30); stmt.bindout(2,&stcolumns.pkseq);
...
void crtsql() {
  ...
  for (int ii = 0; ii < TABCOLS.m_vallcols.size(); ii++) {
    ...
    if (strcmp(TABCOLS.m_vallcols[ii].datatype, "date") != 0)
      ...
      SNPRINTF(strtemp, 100, sizeof(strtemp),
               "str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')", colseq);
  ...
  SNPRINTF(strinsertsql, 10240, sizeof(strinsertsql),
           "insert into %s(%s) values(%s)", stxmltotable.tname, strinsertp1,
           strinsertp2);
  ...
  // t=:1,p=:2,u=:3,wd=:4,wf=:5,r=:6,vis=:7,upttime=now(),mint=:8,minttime=str_to_date(:9,'%Y%m%d%H%i%s')
  ...
  for (int ii = 0; ii < TABCOLS.m_vpkcols.size(); ii++)
    for (int jj = 0; jj < TABCOLS.m_vallcols.size(); jj++)
      if (strcmp(TABCOLS.m_vpkcols[ii].colname,
                 TABCOLS.m_vallcols[jj].colname) == 0) {
  ...
  for (int ii = 0; ii < TABCOLS.m_vallcols.size(); ii++) {
    ...
    char strtemp[101];
    if (strcmp(TABCOLS.m_vallcols[ii].datatype, "date") != 0)
      SNPRINTF(strtemp, 100, sizeof(strtemp), "%s=:%d",
               TABCOLS.m_vallcols[ii].colname, colseq);
    else
      SNPRINTF(strtemp, 100, sizeof(strtemp),
               "%s=str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')",
               TABCOLS.m_vallcols[ii].colname, colseq);
  ...
  strcat(strupdatesql,
         " where 1=1 ");  // 用1=1是为了后面的拼接方便，这是常用的处理方法。
  ...
  for (int ii = 0; ii < TABCOLS.m_vallcols.size(); ii++) {
    ...
    char strtemp[101];
    if (strcmp(TABCOLS.m_vallcols[ii].datatype, "date") != 0)
      SNPRINTF(strtemp, 100, sizeof(strtemp), " and %s=:%d",
               TABCOLS.m_vallcols[ii].colname, colseq);
    else
      SNPRINTF(strtemp, 100, sizeof(strtemp),
               " and %s=str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')",
               TABCOLS.m_vallcols[ii].colname, colseq);
...
void splitbuffer(char* strBuffer) {
  ...
  for (int ii = 0; ii < TABCOLS.m_vallcols.size(); ii++) {
    ...
    if (strcmp(TABCOLS.m_vallcols[ii].datatype, "date") == 0) {
      GetXMLBuffer(strBuffer, TABCOLS.m_vallcols[ii].colname, strtemp,
                   TABCOLS.m_vallcols[ii].collen);
    ...
    // 如果是字符字段，直接提取。
    GetXMLBuffer(strBuffer, TABCOLS.m_vallcols[ii].colname, strcolvalue[ii],
                 TABCOLS.m_vallcols[ii].collen);
  }
}
